@if (Auth::user()->hasRole('super_admin'))
    <nav class="sb-topnav navbar navbar-expand navbar-primary bg-indigo-800 shadow-sm " name="11">
        <!-- Navbar Brand-->
        <a class="navbar-brand text-light ps-3" href="{{ route('admin.dashboard') }}">Admin Panel</a>
        <!-- Sidebar Toggle-->
...
@elseif (Auth::user()->hasRole('student'))
    <nav class="sb-topnav navbar navbar-expand navbar-dark shadow-sm bg-gray-800 " name="22">
        <!-- Navbar Brand-->
        <a class="navbar-brand ps-3" href="{{ route('student.dashboard') }}">{{ Auth::user()->name }}</a>
        <!-- Sidebar Toggle-->
        <button class="btn btn-link  btn-sm order-1 order-lg-0 me-4 me-lg-0" id="sidebarToggle" href="#!"><i
            ...
            <li class="nav-item dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                    <li><a class="dropdown-item "
                            href="{{ route('student.profile', Auth::user()->student) }}">Profile</a></li>
                    <li>
                        <hr class="dropdown-divider" />
                    </li>
                    <li><a class="dropdown-item" href="{{ route('logout') }}">Logout</a></li>
